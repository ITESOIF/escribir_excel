...
Portafolio_1 = matrix(nrow = p, ncol = 4)
colnames(Portafolio_1) = c("Portafolio_1", "Portafolio_2", "Portafolio_3", "Portafolio_4")
...
pesos1 <- na.omit(pesos1)
pesos2 <- na.omit(pesos2)
pesos3 <- na.omit(pesos3)

for(i in 1:p){
  suma <- 0
  for(j in j:4){
    suma <- suma + (as.numeric(Rendimientos[i,j])*as.numeric(pesos1[i]))
  }
  Portafolio_1[i,j] <- suma #Rendimiento del port original de cada día
}
...
plot_ly(Historicos_Ports) %>%
 add_trace(x = ~Date, y = ~round(Portafolio_1,2), type = 'scatter', mode = 'lines', name = 'Máximo Rendimiento',
  ...
  add_trace(x = ~Date, y = ~round(Portafolio_4,2), type = 'scatter', mode = 'lines', name = 'Portafolio Original',
            line = list(color = 'limegreen'), hoverinfo = "text", text = ~paste('Port_4',round(Portafolio_4,2)))  %>%
