...
#Obteniendo rendimientos históricos de cada portafolio
...
pesos1 <- pesos1[!is.na(pesos1)] #Pesos máximo rendimiento 
pesos2 <- pesos2[!is.na(pesos2)] #Pesos mínima varianza
pesos3 <- pesos3[!is.na(pesos3)] #Pesos máximo sharpe
...
for(i in 1:length(Rendimientos[,1])){
  ...
  rend1[i] <- suma1 #Rendimiento del port max rend de cada día
  rend2[i] <- suma2 #Rendimiento del port min var de cada día
  rend3[i] <- suma3 #Rendimiento del port max sharpe de cada día
  rend4[i] <- suma4 #Rendimiento del port original de cada día
}
...
plot_ly(x = p5$fechas, y = p5$rend1, type = 'scatter', mode = 'lines', name = 'Máximo Rendimiento',
            line = list(color = 'red'), hoverinfo = "text", text = ~paste('Port_1', p5$rend1)) %>%
  add_trace(x = p5$fechas, y = p5$rend2, type = 'scatter', mode = 'lines', name = 'Mínima Varianza',
            line = list(color = 'blue'), hoverinfo = "text", text = ~paste('Port_2', p5$rend2))  %>%
  add_trace(x = p5$fechas, y = p5$rend3, type = 'scatter', mode = 'lines', name = 'Máximo Sharpe Ratio',
  ...
  layout(title = "3 Portafolios distintos  vs Portafolio original",
         ...
         yaxis = list(title = "Evolución del rendimiento"), 
