...
for(i in 1:Peso_Or){
  ...
  if((i==Peso_Or)||(i==VaOr)){
    ...
    suma2 <- 1
    for(j in 1:length(completos)){
    suma <- suma + (as.numeric(p4[VaOr,j])*as.numeric(p4[Peso_Or,j]))^2 
    suma2 <- suma2*(as.numeric(p4[VaOr,j])*as.numeric(p4[Peso_Or,j]))
    }
    mult <- suma + (2*suma2)#Aqui falta agregar la covarianza
    varOr[i] <- mult #Desv Est del port original de cada día
  }
}
...
#A PARTIR DE AQUI ESTO ESTA INCOMPLETO
#Creando el portafolio origial a la fecha más reciente
ipar <- 5
impar <- 4
cont <- 0

Port_4 <- matrix(nrow = 1, ncol = 131)
Port_4[1,1] <- 3#rendOr
Port_4[1,2] <- 4#varOr
Port_4[1,3] <- "No-Frontera"

for(i in 1:131){
  if(i>=4){
    Port_4[1,impar] <-  p4[Peso_Or,cont+1]#Peso activo or
    Port_4[1,ipar] <- 0 #Titulos
    ipar <- ipar+2
    impar <- impar+2
    cont <- cont+1
  }
...
Ports <- cbind(rbind(Port_1, Port_2, Port_3, Port_4),
