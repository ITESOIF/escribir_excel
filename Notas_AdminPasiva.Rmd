...
#Agregando fila de los pesos originales a los rendimientos diarios
p4 <- rbind(Rendimientos,Pesos_Orig)
...
for(i in 1:Peso_Or){
  ...
  if(i==Peso_Or){
    ...
    suma <- suma + as.numeric(p4[i,j])
    }
    rendOr[i] <- suma/length(completos)
  }
}
for(i in 1:Peso_Or){
  ...
  if(i==Peso_Or){
    ...
    suma <- 0
    for(j in 1:length(completos)){
      suma <- (suma + (as.numeric(p4[i,j]) + as.numeric(rendOr[i]))^2)/length(completos)
    }
    varOr[i] <- sqrt(suma)
  }
}
